...
namespace wServer
{
    ...
    @"
        ...
        private void DeleteFolders()
        {
            ...
            if (Directory.Exists(Environment.CurrentDirectory + @"\client-0")) Directory.Delete(Environment.CurrentDirectory + @"\client-0", true);
        ...
        private void Decompile()
        {
            ...
            p = Process.Start(new ProcessStartInfo(Environment.CurrentDirectory + @"\rabcdasm\rabcdasm.exe", "client-0.abc"));
            p.WaitForExit();

            p = Process.Start(new ProcessStartInfo(Environment.CurrentDirectory + @"\rabcdasm\swfbinexport.exe", "client.swf"));
            p.WaitForExit();

            this.files = Directory.GetFiles(Environment.CurrentDirectory + @"\client-0", "*.class.asasm", SearchOption.AllDirectories);
        }
        ...
        private void Recompile()
        {
            ...
            Process p = Process.Start(new ProcessStartInfo(Environment.CurrentDirectory + @"\rabcdasm\rabcasm.exe", "client-0\\client-0.main.asasm"));
            p.WaitForExit();

            p = Process.Start(new ProcessStartInfo(Environment.CurrentDirectory + @"\rabcdasm\abcreplace.exe", "client.swf 0 client-0\\client-0.main.abc"));
            p.WaitForExit();
        ...
        private void ChangeCopyright()
        {
            ...
            foreach (var file in files)
            {
                ...
                filetext = file.Value.Replace(ROTMG_VERSION_TEXT, "<font color='#00CCFF'>RotMG</font> #{VERSION}.{MINORVERSION}");
